...
#define SOFA_RGBDTRACKING_FakeCam_INL
#include <limits>
#include <iterator>
...
template <class DataTypes>
void FakeCam<DataTypes>::handleEvent(sofa::core::objectmodel::Event *event) {
...
template <class DataTypes>
void FakeCam<DataTypes>::writeImages() {

    for (int frame_count = 0 ;frame_count < listimgseg.size()-5; frame_count++) {
        std::cout << "(FakeCam) write image : " << frame_count << std::endl;
...
//        }
    ...
    for (int frame_count = 1 ;frame_count < listrtt.size(); frame_count++) {
        std::cout << "(FakeCam) write rtt : " << frame_count << std::endl;
